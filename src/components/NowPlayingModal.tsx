...
export const NowPlayingModal: React.FC = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#0c080b]/98 backdrop-blur-2xl text-[#f5ebe6] flex flex-col justify-between overflow-y-auto animate-in fade-in duration-300 select-none pb-[env(safe-area-inset-bottom,0px)]">
      ...
      {/* ═══════════════════════════════════════════════════════════════
          MOBILE NOW PLAYING VIEW (Screen 4 from reference)
          ═══════════════════════════════════════════════════════════════ */}
      <div className="md:hidden flex-1 flex flex-col justify-between px-6 pt-5 pb-8 relative z-10 max-w-md mx-auto w-full">
        {/* Mobile Header */}
        <div className="flex items-center justify-between pb-3">
          <button
            onClick={() => toggleFullScreenPlayer(false)}
            className="p-2 -ml-2 text-[#e29d8f] hover:text-white"
            title="Minimize"
          >
          ...
          <button
            ...
            className="p-2 -mr-2 text-[#e29d8f] hover:text-white"
        ...
        {/* Big Album Art Square */}
        <div className="my-auto py-4 flex justify-center">
          <div className="relative w-64 h-64 sm:w-72 sm:h-72 rounded-3xl overflow-hidden shadow-[0_16px_48px_rgba(0,0,0,0.9)] border border-[#e29d8f]/25 bg-[#160d13]">
            <SongCover
        ...
        <div className="flex items-center justify-between gap-4 mt-2">
          <div className="min-w-0 flex-1">
            ...
            <p className="text-sm text-[#e29d8f] font-medium truncate mt-1">
              {currentSong.artist || currentSong.category || 'Pattupetti'}
            </p>
          </div>
          <button
            onClick={() => toggleFavorite(currentSong.id)}
            className="p-2 text-[#ab9398] hover:text-[#e29d8f] transition-colors"
          >
            <Heart className={`w-6 h-6 ${isFav ? 'text-[#e29d8f] fill-[#e29d8f]' : ''}`} />
          </button>
        </div>
        ...
        <div className="space-y-1.5 pt-4">
          <div className="relative flex items-center h-4 group">
            <div className="w-full h-1.5 bg-[#251720] rounded-full overflow-hidden">
              <div
          ...
          <div className="flex justify-between text-xs font-mono text-[#a88d92] tabular-nums">
        ...
        <div className="flex items-center justify-between pt-4 px-2">
          <button
            ...
            className={`p-2 transition-colors ${
              isShuffle ? 'text-[#e29d8f]' : 'text-[#8c7479] hover:text-white'
            }`}
            title="Shuffle"
          >
          ...
          <button
            onClick={prevSong}
            className="p-2 text-[#e29d8f] hover:text-white active:scale-95 transition-transform"
            title="Previous"
          >
          ...
          <button
            onClick={togglePlay}
            className="w-16 h-16 rounded-full bg-[#e29d8f] text-[#0c080b] flex items-center justify-center shadow-xl shadow-[#e29d8f]/30 active:scale-95 transition-transform"
          ...
          <button
            ...
            className={`p-2 transition-colors ${
              isRepeat ? 'text-[#e29d8f]' : 'text-[#8c7479] hover:text-white'
            }`}
            title="Repeat"
          >
        ...
        <div className="pt-4 flex justify-center">
          <div className="flex items-center gap-2 px-4 py-2 rounded-2xl bg-[#1a0f18] border border-[#e29d8f]/20 text-xs font-mono text-[#c4a8ad]">
            <span>Lyrics</span>
            <span className="text-[#e29d8f]">▲</span>
          </div>
        </div>
      </div>

      {/* ═══════════════════════════════════════════════════════════════
          DESKTOP FULL SCREEN VIEW (Vinyl Sleeve Experience)
          ═══════════════════════════════════════════════════════════════ */}
      <div className="hidden md:flex flex-col justify-between flex-1">
        {/* Top Header Bar */}
        <header className="relative z-10 flex items-center justify-between px-12 py-6 border-b border-[#e29d8f]/10">
          <div className="flex items-center gap-3">
            <span className="font-display text-2xl tracking-widest text-white">PATTUPETTI</span>
            <span className="text-[10px] tracking-[0.25em] text-[#e29d8f]/70 uppercase">
              • PERSONAL MUSIC BOX
            </span>
          </div>
        ...
        <main className="relative z-10 flex-1 flex items-center justify-center gap-20 px-16 py-8 max-w-7xl mx-auto w-full">
          ...
          <div className="relative flex items-center justify-center max-w-[420px] w-full">
            {/* Vinyl Record */}
            <div
              className={`absolute right-[-25%] w-80 h-80 vinyl-record vinyl-grooves flex items-center justify-center shadow-2xl transition-all duration-700 ${
                isPlaying ? 'animate-spin-slow' : ''
              }`}
            >
              <div className="w-32 h-32 rounded-full bg-[#170e14] border-4 border-[#e29d8f]/40 flex flex-col items-center justify-center p-2 text-center shadow-inner">
                <Disc3 className="w-6 h-6 text-[#e29d8f] mb-1 opacity-80" />
                <span className="text-[9px] font-display text-white uppercase tracking-wider line-clamp-1">
                  {currentSong.title}
                </span>
            ...
            {/* Album Cover Sleeve */}
            <div className="relative z-10 w-80 h-80 rounded-2xl overflow-hidden shadow-2xl shadow-black/90 border border-[#e29d8f]/20 bg-[#160d13]">
              <SongCover
                url={currentSong.cover_url}
                alt={currentSong.title}
                category={currentSong.category}
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent flex flex-col justify-end p-6">
                <span className="font-script text-4xl text-[#f5bcaf] drop-shadow-md">
                  {currentSong.title}
                </span>
          ...
          <div className="w-full max-w-lg flex flex-col justify-center space-y-6">
            ...
            <div className="border-l-2 border-[#e29d8f]/50 pl-4 py-1">
              ...
              <p className="text-lg text-[#e29d8f] font-medium mt-1">
