...
export const BottomPlayer: React.FC = () => {
  ...
  return (
    <>
      {/* ═══════════════════════════════════════════════════════════════
          MOBILE FIXED MINI PLAYER (Fixed above bottom navigation)
          ═══════════════════════════════════════════════════════════════ */}
      {currentSong && (
        <aside
          aria-label="Now playing bar"
          className="md:hidden fixed left-2.5 right-2.5 bottom-[calc(60px+env(safe-area-inset-bottom,0px))] z-40 bg-[#160c14]/97 backdrop-blur-xl border border-[#e29d8f]/30 rounded-2xl shadow-[0_8px_32px_rgba(0,0,0,0.85)] select-none animate-in slide-in-from-bottom-2 duration-200 overflow-hidden"
        >
          {/* ── Row 1: Cover Art + Song Info (tap to open full player) ── */}
          <div
            onClick={() => toggleFullScreenPlayer(true)}
            className="flex items-center gap-2.5 px-3 pt-2.5 pb-1 cursor-pointer"
          >
            <div className="relative w-10 h-10 rounded-xl overflow-hidden bg-[#1f111a] border border-[#e29d8f]/25 flex-shrink-0 shadow-md">
              <SongCover
              ...
              {isLoadingAudio && (
                <div className="absolute inset-0 bg-black/60 flex items-center justify-center">
                  <div className="w-3.5 h-3.5 border-2 border-[#e29d8f] border-t-transparent rounded-full animate-spin" />
                </div>
              )}
            </div>
            <div className="min-w-0 flex-1">
              <h4 className="text-xs font-bold text-white truncate leading-tight">{currentSong.title}</h4>
              <p className="text-[10px] text-[#e29d8f] truncate font-medium mt-0.5">
                {currentSong.artist || currentSong.category || 'Pattupetti'}
              </p>
            </div>
          </div>

          {/* ── Row 2: 5 Control Buttons: Shuffle · Prev · Play/Pause · Next · Repeat ── */}
          <div className="flex items-center justify-around px-3 py-1.5">
            <button
              onClick={(e) => { e.stopPropagation(); toggleShuffle(); }}
              className={`p-2 rounded-full transition-all active:scale-90 ${isShuffle ? 'text-[#e29d8f]' : 'text-[#6a5560] hover:text-[#c4a8ad]'}`}
              aria-label="Shuffle"
            >
            ...
            <button
              ...
              className="p-2 text-[#e29d8f] hover:text-white active:scale-90 transition-transform"
            ...
            <button
              ...
              className="w-11 h-11 rounded-full bg-[#e29d8f] text-[#0c080b] flex items-center justify-center shadow-lg shadow-[#e29d8f]/30 active:scale-90 transition-transform"
            ...
            <button
              ...
              className={`p-2 rounded-full transition-all active:scale-90 ${isRepeat ? 'text-[#e29d8f]' : 'text-[#6a5560] hover:text-[#c4a8ad]'}`}
              aria-label="Repeat"
            >
          ...
          <div className="flex items-center gap-2 px-3 pb-2.5 pt-0.5">
            <span className="text-[9px] font-mono text-[#a88d92] w-7 text-right tabular-nums shrink-0">
              {formatTime(currentTime)}
            </span>
            <div className="relative flex-1 flex items-center h-3">
              <div className="w-full h-[3px] bg-[#251720] rounded-full overflow-hidden">
                <div
                  className="h-full bg-gradient-to-r from-[#d77c6b] to-[#e29d8f] rounded-full transition-all duration-75"
            ...
            <span className="text-[9px] font-mono text-[#a88d92] w-7 text-left tabular-nums shrink-0">
              {formatTime(duration)}
            </span>
          </div>
        </aside>
      ...
      {/* ═══════════════════════════════════════════════════════════════
          DESKTOP WORKSTATION FOOTER (Screen 07)
          ═══════════════════════════════════════════════════════════════ */}
      <footer className="hidden md:flex fixed bottom-0 left-0 right-0 h-24 bg-[#0d090c]/95 backdrop-blur-2xl border-t border-[#e29d8f]/15 z-40 px-8 items-center justify-between transition-all select-none shadow-2xl">
